    ...
    def run_loop(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for motion, cond in tqdm(self.data):
                ...
                motion = motion.to(self.device) # torch.Size([64, 72, 1, 81])
                cond['y'] = {key: val.to(self.device) if torch.is_tensor(val) else val for key, val in cond['y'].items()} # dict_keys(['mask', 'lengths', 'other_motion'])
