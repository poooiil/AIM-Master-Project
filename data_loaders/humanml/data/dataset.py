...
class PW3D_Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, splits, w_vectorizer, **kwargs):
        ...
        self.pointer = 0
        self.max_motion_length = opt.max_motion_length
        self.min_motion_len = 100 if self.opt.dataset_name =='self_humanml' else 24
        # self.canon_relevant_entries = [0, 2, 6, 8]

        data_dict = {}
        base_dir = '/dataset/3dpw/joint_vecs' # './dataset/3dpw/joint_vecs'
        splits = splits.split(',')
        ...
        for name in tqdm(id_list):
            ...
            def get_canon(_name):
                _canon = np.load(_name.replace('joint_vecs', 'canon_data')) / 10.
                # print("Check canono dim:", _canon.shape)
                return np.concatenate((_canon, np.zeros(self.opt.dim_pose-9)))
            canon0 = get_canon(name0)
            canon1 = get_canon(name1)
            if not 'test' in splits:  # test is not yet annotated
                with open(name0.replace('joint_vecs', 'text_chatgpt').replace('p0', 'p0').replace('.npy', '.txt'), 'r') as fr:# 'p0_M'
    ...
    def __getitem__(self, item):
        ...
        offset = 0# random.randint(0, orig_length-m_length-1)
        ...
        caption, tokens = random.choice(text_data['caption'][:4]), text_data['tokens']
        ...
        if self.opt.load_mode == 'text':
            if m_length < self.max_motion_length:
                ...
                other_motion = np.concatenate([other_motion,
                                         np.zeros((self.max_motion_length - m_length, other_motion.shape[1], other_motion.shape[2]))
                                         ], axis=0)
        # print("看一下数据的维度:", motion.shape, other_motion.shape)
        "Z Normalization"
        ...
        # return word_embeddings, pos_one_hots, caption, sent_len, motion, m_length, '_'.join(tokens)
        # print("看一下数据的维度:", motion.shape, other_motion.shape)
        # print("CANONO dim : ", canon.shape)
        # assert 1==2
        motion, other_motion = motion.reshape(120,-1), other_motion.reshape(120, -1)
        # Concat canon data
