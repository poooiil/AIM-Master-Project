...
from model.my_mdm import my_MDM
...
def main():
    ...
    if args.text_prompt != '':
        print("TEXT PROMPT!!!---", args.text_prompt)
    ...
    if not args.sample_gt:
        print("加载权重~~")
        model, diffusion = load_model(args, data, dist_util.dev(), ModelClass=my_MDM)
    else:
        model, diffusion = create_model_and_diffusion(args, data, ModelClass=my_MDM)
    ...
    for rep_i in range(args.num_repetitions):
        ...
        dict_motion = {}
        dict_motion['left'], dict_motion['right'] = sample[0,:,0,:].unsqueeze(0).unsqueeze(2), sample1[0,:,1,:].unsqueeze(0).unsqueeze(2)
        # dict_motion['left'], dict_motion['right'] = sample[0].unsqueeze(0), sample1[0].unsqueeze(0)
        print("堪你的维度：", dict_motion['left'].shape)
        # assert 2==3
        np.save("/workspace/priorMD/temporary_folder/test_our_chatgptData/samples/data/sample_{}_{}.npy".format(name.split('.')[0], rep_i), dict_motion)
        print("能否进行到这里？？？")
